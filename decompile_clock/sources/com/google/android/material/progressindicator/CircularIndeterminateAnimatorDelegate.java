...
final class CircularIndeterminateAnimatorDelegate extends IndeterminateAnimatorDelegate<ObjectAnimator> {
    ...
    private static final int[] DELAY_TO_EXPAND_IN_MS = {0, R2.attr.layout_flexBasisPercent, R2.color.common_timer_item_num_color, R2.color.miuix_appcompat_spinner_fg_color_light};
    private static final int[] DELAY_TO_COLLAPSE_IN_MS = {R2.attr.colorOnSurfaceQuaternary, R2.attr.searchEditTextHintColor, R2.color.material_personalized_color_secondary_text_inverse, R2.color.miuix_preference_connect_summary_disconnected_color};
    private static final int[] DELAY_TO_FADE_IN_MS = {1000, R2.attr.textureBlurFactor, R2.color.miuix_appcompat_dropdown_popup_list_item_bg_pressed_light, R2.dimen.abc_dropdownitem_text_padding_right};
    ...
    private void updateSegmentPositions(int i) {
        ...
        for (int i2 = 0; i2 < 4; i2++) {
            activeIndicator.endFraction += this.interpolator.getInterpolation(getFractionInRange(i, DELAY_TO_EXPAND_IN_MS[i2], R2.attr.colorOnSurfaceQuaternary)) * 250.0f;
            activeIndicator.startFraction += this.interpolator.getInterpolation(getFractionInRange(i, DELAY_TO_COLLAPSE_IN_MS[i2], R2.attr.colorOnSurfaceQuaternary)) * 250.0f;
        }
    ...
    private void maybeUpdateSegmentColors(int i) {
        for (int i2 = 0; i2 < 4; i2++) {
            float fractionInRange = getFractionInRange(i, DELAY_TO_FADE_IN_MS[i2], R2.attr.arrowShaftLength);
