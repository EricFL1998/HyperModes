...
public final class Util {
    ...
    public static String hostHeader(HttpUrl httpUrl, boolean z) {
        String strHost;
        if (httpUrl.host().contains(MethodCodeHelper.IDENTITY_INFO_SEPARATOR)) {
            strHost = "[" + httpUrl.host() + "]";
        } else {
            strHost = httpUrl.host();
        }
        return (z || httpUrl.port() != HttpUrl.defaultPort(httpUrl.scheme())) ? strHost + MethodCodeHelper.IDENTITY_INFO_SEPARATOR + httpUrl.port() : strHost;
    }
    ...
    private static InetAddress decodeIpv6(String str, int i, int i2) {
        ...
        while (i < i2) {
            ...
            if (i5 != 0) {
                if (!str.regionMatches(i, MethodCodeHelper.IDENTITY_INFO_SEPARATOR, 0, 1)) {
