...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM1765constructorimpl;
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM1765constructorimpl = SegmentOrClosed.m1765constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM1765constructorimpl = SegmentOrClosed.m1765constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m1770isClosedimpl(objM1765constructorimpl)) {
                Segment segmentM1768getSegmentimpl = SegmentOrClosed.m1768getSegmentimpl(objM1765constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= segmentM1768getSegmentimpl.getId()) {
                        break loop0;
                    }
                    if (!segmentM1768getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, segmentM1768getSegmentimpl)) {
                    ...
                    if (segmentM1768getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM1768getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m1768getSegmentimpl(objM1765constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, segmentM1768getSegmentimpl)) {
                    if (!segment.decPointers$kotlinx_coroutines_core()) {
                        break loop0;
                    }
                    segment.remove();
